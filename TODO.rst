...
-  add filter to only consider “new” versions that are “stable” where
   ...
   -  for instance, what prompted this todo entry is that running
      ``cargo upgrade`` in a rust project with the dependency
      ``chacha20 = { version = "0.9.1", features = ["std"] }`` caused
      the ``chacha20`` dependency to be bumped to
      ``chacha20 = { version = "0.10.0-rc.6", features = ["std"] }``
      which appears to be a release-canditate and, at any rate, caused
      the dependent project (mu-cli) to fail with the following output:
